...
function addRecord() {
    ...
    var monarch = $("#monarch").val();
    var title = $("#title").val().trim();
    var n = i = 1;
    var count = $("#item_content").children().length;
	// var id_master = id_item_stock = stock_name = buy = sell = qty = new Array(count);
	id_item_stock = new Array(count); qty = new Array(count);
    while(i<=count){
        var str = ["#id_item_stock_"+n, "#qty_"+n, "#id_panel_"+n];
        if($(str[0]).val() == null){
            n++;
        }
        else{
            id_item_stock.push($(str[0]).val());
            qty.push($(str[1]).val());
            $(str[2]).remove();
    ...
    if (monarch == "" && title == "" ) {
        ...
        // Add record
        $.post("../../ajax/acct/shop/billing/create.php", {
            title:title,
            count:count,
            monarch:monarch,
...
function readRecords() {
    ...
    $.post("../../ajax/acct/shop/billing/read.php", {
        type_data:type_data
    }, function (data, status) {
...
function DeleteUser(id) {
    ...
    if (conf == true) {
        $.post("../../ajax/acct/shop/billing/delete.php", {
                id: id
            },
            function (data, status) {
...
function moveToPaid(id){
    var conf = confirm("Move Billing to Paid-Bill?");
    if (conf == true) {
        $.post("../../ajax/acct/shop/billing/move-to-paid.php", {
                id: id
            },
            function (data, status) {
                // reload Users by using readRecords();
...
function itemStock(n){
    var id = "#item_stock_"+n;
    var id2 = "#qty_content_"+n;
    var id_master = "#id_master_"+n;
    id_master = $(id_master).val();
    $.post("../../ajax/acct/shop/billing/current-stock.php", {
        id:n,
        id_master:id_master,
        type:'1'
    }, function (data, status) {
...
function u_itemStock(n){
    var id = "#u_item_stock_"+n;
    var id2 = "#u_qty_content_"+n;
...
function u_addQty(n){
    var id = "#u_qty_content_"+n;
    var id_item_stock = "#u_id_item_stock_"+n;
    var id_billing = $("#hidden_user_id").val();
    id_item_stock = $(id_item_stock).val();
    $.post("../../ajax/acct/shop/billing/qty-content.php", {
        id:n,
...
function addItem(){
    item_number ++;
    $.post("../../ajax/acct/shop/billing/add-item.php", {
...
function GetUserDetails(id) {
    ...
    $("#u_item_content").html("");
    item_number = 0;
    var total_item = 0, bill = null;
    $("#hidden_user_id").val(id);

    $.post("../../ajax/acct/shop/billing/read-total-billing.php", {
            id: id
        },
        function (data, status) {
            // PARSE json data
    ...
    $.post("../../ajax/acct/shop/billing/details-item-billing.php", {
        id: id
    },
    function (data, status) {
...
$.post("../../ajax/acct/shop/billing/details.php", {
    id: id
},
function (data, status) {
    // PARSE json data
...
function UpdateUserDetails() {
	...
	id_item_stock = new Array(count); qty = new Array(count);
    while(i<=count){
        var str = ["#u_id_item_stock_"+n, "#u_qty_"+n, "#u_id_panel_"+n];
        if($(str[0]).val() == null){
            n++;
        }
        else{
    ...
    if (monarch == "" && title == "" ) {
        ...
        $.post("../../ajax/acct/shop/billing/update.php", {
            title:title,
            count:count,
            monarch:monarch,
            id_item_stock:id_item_stock,
...
function u_addItem(i, bill){
    $.post("../../ajax/acct/shop/billing/add-item.php", {
        item_number:i,
        type:'2',
        id_item_stock:bill.id_item_stock,
        qty:bill.qty,
    }, function (data, status) {
        $("#u_item_content").append(data);
        // u_readItemMaster(i);
